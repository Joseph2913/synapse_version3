...
| `get_full_graph(p_user_id)` | Fetches all nodes (with pre-computed x/y positions) and all edges for the full graph view. Positions are computed by `api/graph/compute-layout.ts` after each ingestion. | `src/services/graphQueries.ts` → `fetchFullGraph()` |
