...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !GEMINI_API_KEY) {
  throw new Error('[gemini/extract] Missing required env vars')
}
...
function isBody(b: unknown): b is ExtractBody {
  if (!b || typeof b !== 'object') return false
  const o = b as Record<string, unknown>
  return typeof o.content === 'string' && o.content.length > 0
    && typeof o.systemPrompt === 'string' && o.systemPrompt.length > 0
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!isBody(req.body)) {
    return res.status(400).json({ error: 'invalid_request', detail: 'Expected { content: string, systemPrompt: string }' })
  }
