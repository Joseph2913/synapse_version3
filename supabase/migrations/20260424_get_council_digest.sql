-- Migration: get_council_digest RPC function
-- Purpose: Weekly digest for the Home view. Computes aggregate stats and top items
-- (tensions, frontier questions, gaps, active agents) across the Council in the
-- last N days. All ranking and filtering happen in Postgres; the browser receives
-- one compact JSON object.
--
-- Called from: src/services/supabase.ts -> fetchCouncilDigest()

CREATE OR REPLACE FUNCTION get_council_digest(
  p_user_id UUID,
...
BEGIN
  -- ── Summary counts (all types, active/open only) ───────────────────────────
  ...
  -- Count of distinct agents that produced anything in the window
  ...
  -- ── Top 3 tensions: highest confidence, then newest ────────────────────────
  ...
  -- ── Top 3 frontier questions: priority first, then newest ──────────────────
  ...
  -- ── Top 3 gaps: significant before moderate, then newest ───────────────────
  ...
  -- ── Active agents: top 5 by combined new items in window ───────────────────
...
GRANT EXECUTE ON FUNCTION get_council_digest(UUID, INT) TO authenticated;
